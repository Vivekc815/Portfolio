...
export default function OilPaintTrail() {
  ...
  useEffect(() => {
    ...
    function draw() {
      if (!ctx) return;
      ctx.globalAlpha = 0.10; // more blending, less darkness
      ctx.fillStyle = "#fff";
      ctx.fillRect(0, 0, width, height);
      ctx.globalAlpha = 1;
      if (mouse.current.isMoving) {
        hue = (hue + 0.2) % 360; // even slower color change
        // More smoothing for slower movement
        ...
        ctx.strokeStyle = `hsl(${hue}, 90%, 80%)`; // lighter color
        ctx.lineWidth = 80; // much larger trail
